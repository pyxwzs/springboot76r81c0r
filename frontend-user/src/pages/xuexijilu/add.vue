...
<template>
	<div class="add-update-preview">
		<el-form
			...
			label-width="200px"
			>
			<el-form-item class="add-item" label="记录编号" prop="jilubianhao">
				<el-input v-model="ruleForm.jilubianhao" placeholder="记录编号" disabled></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="学习时长min" prop="xuexishizhang">
				<el-input-number v-model="ruleForm.xuexishizhang" placeholder="学习时长min" :disabled="ro.xuexishizhang"></el-input-number>
			</el-form-item>
			<el-form-item class="add-item"  label="完成情况" prop="wanchengqingkuang">
				<el-select v-model="ruleForm.wanchengqingkuang" placeholder="请选择完成情况" :disabled="ro.wanchengqingkuang"  filterable>
					<el-option
			...
			<el-form-item class="add-item" label="学习笔记" prop="xuexibiji">
				<el-input
					type="textarea"
					:rows="8"
					:disabled="ro.xuexibiji"
					placeholder="学习笔记"
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`xuexijilu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit(subMitType=null) {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`xuexijilu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
