	...
	<div>
		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<div class="list-btn-box">
					...
					<el-button class="list-add-btn" v-if="btnAuth('xuexijilu','新增')" type="primary" @click="add('/index/xuexijiluAdd')">
						<span class="icon iconfont icon-tianjia7"></span>
						新增
					</el-button>
  					<el-button class="list-static-btn" v-if="btnAuth('xuexijilu','学习时长统计')" type="success" @click="chartDialogShow1">
						<span class="icon iconfont " ></span>
						学习时长统计
					</el-button>
					<el-button class="list-static-btn" v-if="btnAuth('xuexijilu','完成情况统计')" type="success" @click="chartDialogShow2">
						<span class="icon iconfont " ></span>
						完成情况统计
					</el-button>
				...
				<div class="list5">
					<div v-for="(item,index) in dataList" :key="index" class="list-item" @click.stop="toDetail(item)" >
						...
						<div class="infoBox">
							...
							<div class="name">学习时长min:{{item.xuexishizhang}}</div>
							<div class="name">完成情况:{{item.wanchengqingkuang}}</div>
		...
		<el-dialog
			...
			<div v-if="changeStatQuery(['users'])" style="display: flex;width: 100%;align-items: center;margin: 0 0 20px;">
				<div style="padding: 0 10px;">课程名称</div>
				<el-select v-model="chartQuery1.kechengmingcheng" placeholder="请选择" @change="chartDialog1" clearable>
		...
		<el-dialog
			...
			width="800">
			<div id="wanchengqingkuangChart2" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				this.$http.get(`xuexijilu/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/xuexijiluDetail', query: params});
			},
			...
			chartDialog1() {
				this.$nextTick(()=>{
					...
					this.$http.get("xuexijilu/value/xingming/xuexishizhang",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
			...
			chartDialog2() {
				this.$nextTick(()=>{
					...
					let params = {}
					this.$http.get("xuexijilu/typeStat/wanchengqingkuang/kechengmingcheng",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis1 = [];
