	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'：'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/xuexijilu?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">记录编号</div>
						<div class="text "  >{{detail.jilubianhao}}</div>
					</div>
					<div class="item">
						<div class="lable">记录时间</div>
						<div class="text "  >{{detail.jilushijian}}</div>
					</div>
					<div class="item">
						<div class="lable">学习时长min</div>
						<div class="text "  >{{detail.xuexishizhang}}</div>
					</div>
					<div class="item">
						<div class="lable">学习笔记</div>
						<div class="text "  >{{detail.xuexibiji}}</div>
					</div>
					<div class="item">
						<div class="lable">完成情况</div>
						<div class="text "  >{{detail.wanchengqingkuang}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('xuexijilu','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('xuexijilu','删除')" @click="delClick">删除</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('xuexijilu','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/xuexijilu', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/xuexijiluAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此学习记录？') .then(_ => {
					this.$http.post('xuexijilu/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
			...
			allchapterClick (){
				...
				this.$router.push({path: '/index/chapterxuexijilu', query: params});
			},
			// 获取uuid
			getUUID() {
