	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'：'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/examstatistics?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">试卷名称</div>
						<div class="text "  >{{detail.papername}}</div>
					</div>
					<div class="item">
						<div class="lable">考试编号</div>
						<div class="text "  >{{detail.examno}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('examstatistics','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('examstatistics','删除')" @click="delClick">删除</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('examstatistics','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/examstatistics', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/examstatisticsAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此测试统计？') .then(_ => {
					this.$http.post('examstatistics/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
			...
			allchapterClick (){
				...
				this.$router.push({path: '/index/chapterexamstatistics', query: params});
			},
			// 获取uuid
			getUUID() {
