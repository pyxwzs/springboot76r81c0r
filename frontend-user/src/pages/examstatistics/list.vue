<template>
	<div>
		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				<el-form-item class="list-item" >
					...
					<el-input v-model="formSearch.papername" placeholder="试卷名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<div class="list-btn-box">
					...
					<el-button class="list-add-btn" v-if="btnAuth('examstatistics','新增')" type="primary" @click="add('/index/examstatisticsAdd')">
						<span class="icon iconfont icon-tianjia7"></span>
						新增
					</el-button>
  					<el-button class="list-static-btn" v-if="btnAuth('examstatistics','试卷平均成绩')" type="success" @click="chartDialogShow1">
						<span class="icon iconfont " ></span>
						试卷平均成绩
					</el-button>
					<el-button class="list-static-btn" v-if="btnAuth('examstatistics','成绩区间分布')" type="success" @click="chartDialogShow2">
						<span class="icon iconfont " ></span>
						成绩区间分布
					</el-button>
					<el-button class="list-static-btn" v-if="btnAuth('examstatistics','学生平均成绩')" type="success" @click="chartDialogShow3">
						<span class="icon iconfont " ></span>
						学生平均成绩
					</el-button>
				...
				<el-table class="tables" :stripe='false'
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="table-btn5" type="success" @click.native="chapterClick(scope.row)" v-if="btnAuth('examstatistics','章节管理')">
		...
		<el-dialog
			...
			width="800">
			<div id="totalscoreChart2" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="totalscoreChart3" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				let searchWhere = {};
				if (this.formSearch.papername != '') searchWhere.papername = '%' + this.formSearch.papername + '%';
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`examstatistics/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/examstatisticsDetail', query: params});
			},
			...
			chartDialog1() {
				this.$nextTick(()=>{
					...
					let params = {}
					params.func = '平均'
					this.$http.get("examstatistics/value/papername/totalscore",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].papername);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].papername
								})
							}
							var option = {};
							let titleObj = this.line.title
							titleObj.text = '试卷平均成绩'
			...
			chartDialog2() {
				this.$nextTick(()=>{
					var totalscoreChart2 = echarts.init(document.getElementById("totalscoreChart2"),'macarons');
					let params = {}
					this.$http.get("examstatistics/group/totalscore",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
							...
							// 使用刚指定的配置项和数据显示图表。
							totalscoreChart2.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								totalscoreChart2.resize();
							};
						}
					})
				})
			...
			chartDialog3() {
				this.$nextTick(()=>{
					var totalscoreChart3 = echarts.init(document.getElementById("totalscoreChart3"),'macarons');
					let params = {}
					params.func = '平均'
					this.$http.get("examstatistics/value/username/totalscore",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
							...
							// 使用刚指定的配置项和数据显示图表。
							totalscoreChart3.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								totalscoreChart3.resize();
							};
						}
					})
				})
