...
<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="试卷名称" prop="papername">
				<el-input v-model="ruleForm.papername" 
					placeholder="试卷名称" clearable :readonly="ro.papername"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="考试编号" prop="examno">
				<el-input v-model="ruleForm.examno" 
					placeholder="考试编号" clearable :readonly="ro.examno"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`examstatistics/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit(subMitType=null) {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`examstatistics/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
