		...
		<el-card style="width: 90%;margin: 10px auto;" v-if="!end||isSubmit">
			<div class="question-content" v-for="(item, index) in questionList" :key="item.id">
				<div class="question-title">
					{{index + 1}} 、({{item.score}}分)<span class="ql-snow ql-editor" v-html="item.questionname"></span>
		...
		methods: {
			...
			async submitAnswer() {
				...
				for(let i in this.questionList){
					if(this.questionList[i].type==1){
						if(!this.questionList[i].myanswer||(this.questionList[i].myanswer&&this.questionList[i].myanswer.length==0)){
							this.$message.error('还有试题名称未完成作答')
							return false
						}
					}else{
				...
				this.$confirm(`是否完成作答，提交在线测试？`, "提示", {
					...
					for (let x in this.questionList) {
						...
						await this.saverecord(this.questionList[x])
					}
					this.end = true;
					if(!this.hassubject) {
						await this.$http.post('examstatistics/add', {
							username: localStorage.getItem("frontExamName")?localStorage.getItem("frontExamName"):'',
							paperid: this.paper.id,
							papername: this.paper.name,
							examno: this.examno,
							totalscore: this.scoreChange(),
							userid: localStorage.getItem("frontUserid"),
						}).then(rs=>{})
					}
					this.$message.success('考试记录已生成')
				}).finally(() => {
			...
			saverecord(row) {
				let record = {
					...
					type: row.type,
					ismark: this.hassubject ? 1 : 0,
					examno: this.examno,
				}
				return this.$http.post('examrecord/add', record)
			},
			doneExam(type) {
				if (type == 0) {
...
<style rel="stylesheet/scss" lang="scss" scoped>
	...
	.question-answer .el-select,
	.question-answer .el-input {
