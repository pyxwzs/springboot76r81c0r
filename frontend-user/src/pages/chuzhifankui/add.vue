	...
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="隐患编号" prop="yinhuanbianhao">
				<el-input v-model="ruleForm.yinhuanbianhao" 
					placeholder="隐患编号" clearable :readonly="ro.yinhuanbianhao"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="处理结果" prop="chulijieguo">
				<el-input v-model="ruleForm.chulijieguo" 
					placeholder="处理结果" clearable :readonly="ro.chulijieguo"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="处理时间" prop="chulishijian">
				<el-date-picker
					:disabled="ro.chulishijian"
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`chuzhifankui/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit(subMitType=null) {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`chuzhifankui/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
