			...
			<template >
				<el-form-item class="input" v-if="type!='info'"  label="名称" prop="name" >
					<el-input
						...
						:placeholder="isCarouselPage ? '轮播项名称（可自定义，如首页一）' : '名称'"
						clearable
						:readonly="ro.name || !isCarouselPage"
					></el-input>
				</el-form-item>
...
<script>
	...
	export default {
		...
		computed: {
			...
			/** 路由 /config/1 为轮播图管理，允许修改「名称」；其它类型配置仍锁定名称 */
			isCarouselPage() {
				const st = this.parent && this.parent.configStatus;
				return st == 1 || st === '1';
			},
		},
		components: {
