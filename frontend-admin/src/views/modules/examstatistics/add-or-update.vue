	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="试卷名称" prop="papername" >
					<el-input v-model="ruleForm.papername" placeholder="试卷名称" clearable  :readonly="ro.papername"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="试卷名称" prop="papername" >
					<el-input v-model="ruleForm.papername" placeholder="试卷名称" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="考试编号" prop="examno" >
					<el-input v-model="ruleForm.examno" placeholder="考试编号" clearable  :readonly="ro.examno"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="考试编号" prop="examno" >
					<el-input v-model="ruleForm.examno" placeholder="考试编号" readonly></el-input>
				</el-form-item>
			</template>
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='paperid'){
							...
							this.ro.paperid = true;
							continue;
						}
						if(o=='papername'){
							this.ruleForm.papername = obj[o];
							this.ro.papername = true;
							continue;
						}
						if(o=='examno'){
							this.ruleForm.examno = obj[o];
							this.ro.examno = true;
							continue;
						}
						if(o=='totalscore'){
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `examstatistics/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.examstatisticsCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
